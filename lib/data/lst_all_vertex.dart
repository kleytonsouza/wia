import 'package:wia/models/point.dart';

final List<Point> allVertex = [
  Point(
    id: 0,
    name: "Selecione um destino",
    coordinate: ['00.00', '00.00'],
  ),
  Point(
    id: 1,
    name: "Atendimento PROGRAD",
    // coordinate: ['-25.4288948', '-49.2677837'],
    coordinate: ['-25.42891780','-49.26780143'],
  ),
  Point(
    id: 2,
    name: "Biblioteca",
    //coordinate: ['-25.428810006', '-49.26774100'],
    coordinate: ['-25.42879171','-49.26766299'],
  ),
  Point(
    id: 3,
    name: "COPAP",
    //coordinate: ['-25.42937111', '-49.26754533'],
    coordinate: ['-25.42938526','-49.26755379'],
  ),
  Point(
    id: 4,
    name: "CIPEAD",
    //coordinate: ['-25.42933392', '-49.26767055'],
    coordinate: ['-25.42935469','-49.26766968'],
  ),
  Point(
    id: 5,
    name: "COAFE",
    //coordinate: ['-25.42928025', '-49.26741163'],
    coordinate: ['-25.42930129','-49.26739021'],
  ),
  Point(
    id: 6,
    name: "COSIS",
    coordinate: ['-25.42885259', '-49.26787655'],
  ),
  Point(
    id: 7,
    name: "Entrada/Saida Principal",
    coordinate: ['-25.42903573', '-49.26746404'],
  ),
  Point(
    id: 8,
    name: "Entrada/Saida Lateral",
    coordinate: ['-25.42876089', '-49.26780696'],
  ),
  Point(
    id: 9,
    name: "joinPoint",
    // coordinate: ['-25.4290562', '-49.2675864'],
    coordinate: ['-25.42907881','-49.26757526'],
  ),
  Point(
    id: 10,
    name: "joinPoint",
    // coordinate: ['-25.4291833', '-49.2675305'],
    coordinate: ['-25.42920535','-49.26751791'],
  ),
  Point(
    id: 11,
    name: "joinPoint",
    // coordinate: ['-25.4291966', '-49.2675567'],
    coordinate: ['-25.42921527','-49.26754625'],
  ),
  Point(
    id: 12,
    name: "joinPoint",
    // coordinate: ['-25.4292777','-49.2675200'],
    coordinate: ['-25.42929175','-49.26751113',],
  ),
  Point(
    id: 13,
    name: "joinPoint",
    // coordinate: ['-25.4292930', '-49.2675117'],
    coordinate: ['-25.42930646','-49.26750262',],
  ),
  Point(
    id: 14,
    name: "joinPoint",
    // coordinate: ['-25.4292981', '-49.2675753'],
    coordinate: ['-25.42931300','-49.26756496',],
  ),
  Point(
    id: 15,
    name: "joinPoint",
    // coordinate: ['-25.4293716', '-49.2675388'],
    coordinate: ['-25.42937973','-49.26753604',],
  ),
  Point(
    id: 16,
    name: "joinPoint",
    // coordinate: ['-25.42935418', '-49.26748866'],
    coordinate: ['-25.42935851','-49.26748110',],
  ),
  Point(
    id: 17,
    name: "joinPoint",
    // coordinate: ['-25.4289675','-49.2677430'],
    coordinate: ['-25.42898900','-49.26773142'],
  ),
  Point(
    id: 18,
    name: "joinPoint",
    // coordinate: ['-25.4289355', '-49.2676561'],
    coordinate: ['-25.42895353','-49.26764357',],
  ),
  Point(
    id: 19,
    name: "joinPoint",
    // coordinate: ['-25.4288692', '-49.2677273'],
    coordinate: ['-25.42888654','-49.26771437'],
  ),
  Point(
    id: 20,
    name: "joinPoint",
    // coordinate: ['-25.4288914', '-49.2677773'],
    coordinate: ['-25.42890689','-49.26776861',],
  ),
  Point(
    id: 21,
    name: "joinPoint",
    // coordinate: ['-25.4288124', '-49.2677515'],
    coordinate: ['-25.42882306','-49.26774672',],
  ),
  Point(
    id: 22,
    name: "joinPoint",
    // coordinate: ['-25.4288239', '-49.2677763'],
    coordinate: ['-25.42883181','-49.26776901'],
  ),
  Point(
    id: 23,
    name: "joinPoint",
    // coordinate: ['-25.4288343', '-49.2678039'],
    coordinate: ['-25.42884200','-49.26779557',],
  ),
  Point(
    id: 24,
    name: "joinPoint",
    // coordinate: ['-25.4288634', '-49.2678730'],
    coordinate: ['-25.42887101','-49.26786721',],
  ),
  Point(
    id: 25,
    name: "joinPoint",
    // coordinate: ['-25.4289478', '-49.2676895'],
    coordinate: ['-25.42896558','-49.26767531',],
  ),
  Point(
    id: 26,
    name: "joinPoint",
    // coordinate: ['-25.42927164', '-49.26744471'],
    coordinate: ['-25.42928383','-49.26743406',],
  ),
  Point(
    id: 27,
    name: "joinPoint",
    // coordinate: ['-25.42927164', '-49.26744471'],
    coordinate: ['-25.42888366','-49.26790141',],
  ),
  Point(
    id: 28,
    name: "joinPoint",
    // coordinate: ['-25.42927164', '-49.26744471'],
    coordinate: ['-25.42900906','-49.26783690',],
  ),
  Point(
    id: 29,
    name: "joinPoint",
    // coordinate: ['-25.42927164', '-49.26744471'],
    coordinate: ['-25.42901322','-49.26779869',],
  ),
  Point(
    id: 30,
    name: "joinPoint",
    // coordinate: ['-25.42927164', '-49.26744471'],
    coordinate: ['-25.42899674','-49.26780693',],
  ),
];
